...
next
  ...
  then show ?case proof(cases "v1 = a")
    ...
    have 2: "v2 \<in> set (a#L)" 
      using assms Cons.prems(3) aux41(2) by force 
    have "v2 \<noteq> a" using True assms aux41_a by fastforce
...
next
  ...
  next
    case False
    then show ?thesis using Cons aux105 by fast
  qed
qed
  ...
  next
    ...
    (\<exists>v e1.
        ...
        (\<exists>e2. v2 = Edge v e2 0 \<and>
              (\<exists>i<length E'. \<exists>j<length E'. e1 = E' ! i \<and> e2 = E' ! j \<and> v \<in> e1 \<and> v \<in> e2 \<and> (\<forall>i'>i. v \<in> E' ! i' \<longrightarrow> i' < length E' \<longrightarrow> \<not> i' < j))))" 
      using Cons by auto
    then show ?thesis using Cons aux110_2 apply(simp)  sorry
  qed
  oops
...
lemma 
  ...
  let u = get_sceond (e - {x}) in (if u \<in> C then [Edge x e 0, Edge x e 1] else [Edge x e 0, Edge u e 0, Edge u e 1, Edge x e 1]) @
        construct_cycle_add_edge_nodes E' x C)" "v1 = Edge u1 e1 0" "v2 = Edge u2 e2 0" "card e \<ge> 2"
  shows "e1 = e2"
proof -
  obtain u where "u = get_second(e-{x})" using assms by simp
  then show ?thesis sorry
  oops*)
    ...
    then show ?thesis proof(cases "u\<in>C")
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge a e 1)]")
        ...
        then have "v2 = hd (tl (construct_cycle_add_edge_nodes (e#E') a C))" using 3 Cons True 4
          by (metis (mono_tags, lifting) append_Cons aux101 list.sel(3))
    ...
    next
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge u e 0), (Edge u e 1), (Edge a e 1)]")
        ...
        then have "v2 = hd (tl (construct_cycle_add_edge_nodes (e#E') a C)) 
          ...
          case True
          then have "v2 = hd (tl (construct_cycle_add_edge_nodes (e#E') a C))" 
            using 3 Cons True 4 aux44_2 by fast
          then show ?thesis ..
        next
          case False
    ...
    then show ?thesis proof(cases "u\<in>C")
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge a e 1)]")
        ...
        then have 5: "v2 = hd (construct_cycle_add_edge_nodes E' a C)" using 3 4
          by (metis (mono_tags, lifting) Cons.prems(1) Cons.prems(4) Cons_eq_appendI aux104 last_ConsL list.set_intros(1))
        then have 6: "v2 \<in> set (construct_cycle_add_edge_nodes E' a C)" 
          using Cons True aux104_1_1 3 4 7
          by fast 
        ...
        next
          case False
          then show ?thesis using aux120_a 5 Cons 
            by metis 
        qed
      next
        case False
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge u e 0), (Edge u e 1), (Edge a e 1)]")
        ...
        next
          ...
          then have 5: "v2 = hd (construct_cycle_add_edge_nodes E' a C)"
            using 3 4 Cons aux104_1 by fast 
          then have 6: "v2 \<in> set (construct_cycle_add_edge_nodes E' a C)" 
            using Cons True aux104_1_1 3 4 7 by fast
          then show ?thesis proof (cases "(construct_cycle_add_edge_nodes E' a C) = []")
            case True
            then show ?thesis using 6 by simp
          next
...
proof -
  ...
  then show ?thesis using aux44_c_3_3_2 assms by auto
qed 
  ...
  then show ?case proof (cases "a\<in>e")
    ...
    then show ?thesis proof(cases "u\<in>C")
      ...
      then show ?thesis proof(cases "Edge a e1 1 \<in> set [(Edge a e 0), (Edge a e 1)]")
        ...
        have "e2 \<in> set E'" using Cons aux44_c_3_3_3 by auto 
        then have e2_set_1: "Edge a e2 1 \<in> set (construct_cycle_add_edge_nodes (E') a C)" using Cons aux44_c_3_3_1 by fast
        then have "Edge a e2 1 \<in> set (construct_cycle_add_edge_nodes (e#E') a C)" using Cons aux44_c_3_3_1 by simp
        then have "\<exists>j1. (construct_cycle_add_edge_nodes (E') a C)! j1 = Edge a e2 1 
          \<and> j1 < length (construct_cycle_add_edge_nodes (E') a C)" using aux44_c_1 e2_set_1 by metis
  ...
  then show ?case proof (cases "a\<in>e")
    ...
    then show ?thesis proof(cases "u\<in>C")
      ...
      then show ?thesis proof(cases "Edge a e1 1 \<in> set [(Edge a e 0), (Edge a e 1)]")
        ...
        then have i1_Edge: "(construct_cycle_add_edge_nodes (e#E') a C)! i1 = Edge a e1 1" using 3 4 by simp
        have "e2 \<in> set E'" using Cons aux44_c_3_3_3 by auto 
        then have e2_set_1: "Edge a e2 0 \<in> set (construct_cycle_add_edge_nodes (E') a C)" using Cons aux44_c_3_3_1b by metis
        then have "Edge a e2 0 \<in> set (construct_cycle_add_edge_nodes (e#E') a C)" using Cons aux44_c_3_3_1 by simp
...
proof -
  ...
  have "\<exists>i. (construct_cycle_add_edge_nodes E' a C)!i = v1 \<and> (construct_cycle_add_edge_nodes E' a C)!(i+1) = v2 \<and> (i+1)< length (construct_cycle_add_edge_nodes E' a C)" 
    using aux44_c_3_2 assms by fast
  ...
  then have "\<exists>i1 j1. (construct_cycle_add_edge_nodes E' a C)! i1 = Edge a e1 1 \<and> (construct_cycle_add_edge_nodes E' a C)! j1 = Edge a e3 1 
    ...
    using aux44_c_3_3 assms e1_def e2_def e3_def 
    by force  
  ...
  then have "\<exists>i1 j1. (construct_cycle_add_edge_nodes E' a C)! i1 = Edge a e3 1 \<and> (construct_cycle_add_edge_nodes E' a C)! j1 = Edge a e2 0 \<and> i1< j1\<and> j1 < length (construct_cycle_add_edge_nodes E' a C)" 
    using aux44_c_3_4 assms e1_def e2_def e3_def by force
...
lemma aux44_c_3:
  ...
  using assms apply(auto) using aux44_c_3_1 assms  
  by metis
    ...
    then show ?thesis proof(cases "u\<in>C")
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge a e 1)]")
        ...
        then have 6: "v2 \<in> set (construct_cycle_add_edge_nodes E' a C)" 
          ...
          by fast 
        then show ?thesis 
          by (metis "4" "5" assms(2) assms(3) aux44_c_2_1 distinct.simps(2) distinct_singleton hc_node.inject(2)) 
      next
        case False
        then have "v1 \<in> set (construct_cycle_add_edge_nodes E' a C)" 
          using 3 Cons aux44_1 by fast
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge u e 0), (Edge u e 1), (Edge a e 1)]")
        ...
        then show ?thesis proof 
          ...
          then have "p1 = [(Edge a e 0), (Edge u e 0)]" 
            using p1p2_def a 
            by (simp add: append_eq_Cons_conv aux104_3 v1_def)  
          then have "v2 = Edge a e 1" 
        ...
        next
          ...
          then show ?thesis using 5 
            by (metis \<open>v1 = Edge a e 1\<close> assms(2) assms(3) aux44_c_2_1 distinct.simps(2) distinct_singleton hc_node.inject(2)) 
        qed
      next
        case False
        then have "v1 \<in> set (construct_cycle_add_edge_nodes E' a C)" 
  ...
  then show ?case proof (cases "a\<in>e")
    ...
    then show ?thesis proof(cases "u\<in>C")
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge a e 1)]")
        case True
        then have 4: "v1 = (Edge a e 1)" using assms by simp
        then have 5: "v1 = last [(Edge a e 0), (Edge a e 1)]" by simp
        then have "v1 \<in> set [(Edge a e 0), (Edge a e 1)]" by simp
    ...
    next
      ...
      then show ?thesis proof(cases "v1 \<in> set [(Edge a e 0), (Edge u e 0), (Edge u e 1), (Edge a e 1)]")
        case True
        then have 4: "v1 = (Edge u e 1) \<or> v1 = Edge a e 1" using assms by simp
        then show ?thesis using assms 3 4 sorry
      next
        case False
        then have "v1 \<in> set (construct_cycle_add_edge_nodes E' a C)" 
          using 3 Cons aux44_1 by fast
  ...
  then show ?case proof(cases "v1 \<in> set (construct_cycle_add_edge_nodes E a C')")
    ...
    next
      ...
      then have 4: "\<exists>p1 p2. p1@ [v1, v2] @ p2 = construct_cycle_add_edge_nodes E a C'" 
        using Cons aux103_4 1 3 11 False by metis
      have "distinct (construct_cycle_add_edge_nodes E a C')" using Cons by simp
      then show ?thesis 
        using Cons aux44 4  by fastforce 
    qed
  next
    case False
    ...
    then have 6: "\<exists>p1 p2. p1@ [v1, v2] @ p2 = construct_cycle_1 E C (Suc n) C'" 
      using Cons aux102_3 11 5 by fast  
...
proof -
  have v1_in_set: "v1 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast
  have "v2 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast 
  then show "e1 \<in> set E" "u1 \<in> e1" using assms v1_in_set in_cycle aux40 by(auto)
next
  show "e1 = e2" "u2 = u1" using aux6_1 assms by blast+
qed
...
next
  ...
  then show ?case proof(cases "v1 \<in> set (construct_cycle_add_edge_nodes E a C')")
    ...
    then show ?thesis proof (cases "v1 = last (construct_cycle_add_edge_nodes E a C')")
      ...
      then have "v2 = hd (construct_cycle_1 E C (Suc n) C')" using Cons aux104_1 1 3 11 True 
        by fast  
      then show ?thesis 
        using Cons by (metis aux43 hc_node.distinct(1)) 
    next
      case False
      have "\<exists>p1 p2. p1@ [v1, v2] @ p2 = tl (construct_cycle_1 E (a#C) n C')" 
        using aux122 Cons
...
proof -
  ...
  have "v2 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast
  then show "e1 \<in> set E" "u1 \<in> e1" using assms v1_in_set in_cycle aux40_a by(auto)
next
  have "v2 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast
  then have u2_in_e2: "u2 \<in> e2"  
    using assms in_cycle aux40_a by(auto)
  then have "e1 = e2" 
    using aux7_1 assms by simp 
...
lemma aux8:
  ...
  shows "u1 = u2" "u1 \<in> e1" "u1 \<in> e2" "e1 \<in> set E" "e2 \<in> set E" 
    "(\<exists>i<length E. \<exists>j<length E. e1 = E ! i \<and> e2 = E ! j \<and> (\<forall>i'>i. u1 \<in> E ! i' \<longrightarrow> i' < length E \<longrightarrow> \<not> i' < j))"
proof -
  have v1_in_set: "v1 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast
  have "v2 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast
  then show "e1 \<in> set E" "u1 \<in> e1" "e2 \<in> set E" using assms v1_in_set in_cycle aux40_a aux40 by(auto)
next
  have "v2 \<in> set (construct_cycle_1 E C n C')" 
    using assms aux41 by fast
  then have u2_in_e2: "u2 \<in> e2"  
    using assms in_cycle aux40 by auto
  then have "u1 = u2 \<and> (\<exists>i<length E. \<exists>j<length E. e1 = E ! i \<and> e2 = E ! j \<and> (\<forall>i'>i. u1 \<in> E ! i' \<longrightarrow> i' < length E \<longrightarrow> \<not> i' < j))"
    using aux8_1 assms  by simp  
    ...
    then show False proof(cases "v2 = hd (construct_cycle_add_edge_nodes E a C')")
      ...
      then show ?thesis proof (cases "(construct_cycle_add_edge_nodes E a C') = []")
        ...
        have "v2 \<in> set ( construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C')" 
          using 1 Cons 
          using aux41(2) by fastforce 
        then have "v2 \<in> set (construct_cycle_1 E C (Suc n) C')" 
          using True by simp
        have "v2 = Cover (n+1) \<or> v2 = Cover 0" using 2 aux43  
      ...
      next
        case False
        then show ?thesis 
          using 2 assms aux44_c_2_1 by force 
      qed
    next
      case False
      then have "v1 \<in> set (construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C')" 
        using Cons 
        by (metis (mono_tags, lifting) "1" \<open>v1 = Cover n\<close> aux101 aux43 distinct.simps(2) hc_node.distinct(1) hd_append list.sel(3)) 
      ...
      then have 3: "\<exists>p1 p2. p1@ [v1, v2] @ p2 = construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C'" 
        using Cons 1 aux106 False 
        by (metis (no_types, lifting) aux43 hc_node.distinct(1) hd_append)  
      then have "v1 \<in> set (construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C')" 
        using Cons 1 
...
proof -
  ...
  have va: "v = a" using assms  
    using aux44_c_2_1 by fastforce 
  ...
  then show ?case proof(cases "v1 = Cover n")
    ...
    then show ?thesis proof(cases "v2 = hd (construct_cycle_add_edge_nodes E a C'@ construct_cycle_1 E C (Suc n) C')")
      ...
      then show ?thesis proof (cases "(construct_cycle_add_edge_nodes E a C') = []")
        ...
        then have "v2 \<in> set (construct_cycle_1 E C (Suc n) C')" 
          using True by simp
        have "v2 = Cover (n+1) \<or> v2 = Cover 0" using 2 True  aux43
          by auto
          then show ?thesis using Cons by auto
      next
        case False
        ...
        then have "\<exists>i<length E. e2 = E! i \<and> u2 \<in> e2 \<and> (\<forall>j. u2 \<in> E ! j \<longrightarrow> j < length E \<longrightarrow> \<not> j < i)" 
          using Cons False aux10_1 ugraph distinct
          by (simp add: Cons.IH \<open>v2 = Edge u2 e2 0\<close> aux10_1) 
        then show ?thesis 
          using Cons False 
          by (metis aux41(1) constraints_for_Cover_nodes length_greater_0_conv list.discI trans_less_add1) 
      qed
    next
      case False
      then have "v1 \<in> set (construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C')" 
        using Cons 
        by (metis (mono_tags, lifting) "1" \<open>v1 = Cover n\<close> aux101 distinct.simps(2) list.sel(3)) 
      then have "distinct (construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C')" using Cons 1 by auto
      then have 3: "\<exists>p1 p2. p1@ [v1, v2] @ p2 = construct_cycle_add_edge_nodes E a C' @ construct_cycle_1 E C (Suc n) C'" 
        using Cons 1 aux106 False 
  ...
  then show ?case proof(cases)
    ...
    show False proof(cases "v1 = last (construct_cycle_add_edge_nodes E a C')")
      ...
      then have "v2 = hd (construct_cycle_1 E C (Suc n) C')" using assms distinct Cons a1 not_empty 
        by (metis (full_types) One_nat_def aux120 aux21 less_one linorder_neqE_nat)
      show ?thesis 
        using aux44_c_2_1b True not_empty assms 
        by fastforce 
    next
      case False
      then have "v2 \<in> set (construct_cycle_add_edge_nodes E a C')" using Cons a1 
...
next  
  ...
  proof(cases)
    ...
    next
      case False
      then show ?thesis 
        using aux12 assms \<open>v1 = _\<close>  \<open>v2 = _\<close> distinct_tl apply(simp) 
        by blast 
    qed
  next
    assume  "\<not> (\<exists> x2. v2 = Cover x2)"

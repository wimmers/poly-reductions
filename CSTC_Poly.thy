...
proof -
  ...
  then have 2: "\<forall>x \<in>X. card x \<le> card (\<Union> X)" using card_x_smaller_card_union assms 
    by (simp add: Sup_upper card_mono)
  from 1 2 assms show ?thesis by(auto simp add: upperbounding_card_union) 
qed

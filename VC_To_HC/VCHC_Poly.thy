...
lemma is_wf_digraph:
  ...
  using F_def
  apply simp
  unfolding first_edge_def next_edge_def last_edge_def wf_digraph_def 
  by fastforce
...
proof -
  have 1: "wf_digraph F"
    using F_def is_wf_digraph 
    by fast
  have "head F = snd" "tail F = fst"
    using F_def
    by auto
  then show ?thesis 
    using arcs_subset_eq_verts_times_verts 1
    by blast
qed
...
proof -
  ...
  then have "k \<le> 2* (length E)"
    using ek_def ugraph_card_verts_smaller_2_length le_trans 
    by blast
  then show ?thesis 
...
lemma fin_set_Cov: 
  assumes "T = {{Cover j|j. j = i}|i. i < k}"
  shows "finite T"
  using assms fin_Cov by simp
...
lemma card_Edge0:
  assumes "M = {Edge v e i|v e. e\<in> set E \<and> v \<in> e}" 
...
lemma fin_verts: 
  ...
  using ek_def 
  unfolding ugraph_def
  using fin_Cov fin_Edge0 fin_Edge1 F_def
  by auto
...
proof -
  have 1: "card (verts F) \<le> 6 * length E" 
    ...
    by auto
  have "card
         (arcs F) \<le> 6 * length E * 6 * length E" 
    using card_arcs  
    by(auto)
  then show ?thesis 
    using 1 

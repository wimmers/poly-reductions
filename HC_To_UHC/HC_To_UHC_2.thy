...
proof -
  ...
  then have "verts G \<subseteq> set c" 
    by auto
  then show ?thesis 
    using card_length card_subset le_trans 
    by blast 
qed
...
proof -
  ...
  then have 1: "set (rev C) \<subseteq> verts G'"
    by simp
  then show "(vwalk_cycle G' (rev C) \<and> (\<forall>x\<in>verts G'. x \<in> set (rev C)) \<and> distinct (tl (rev C)) 
    \<or> card (verts G') \<le> 1) \<and> set (rev C) \<subseteq> verts G'" 
...
proof -
  have 0: "card (verts G') > 1" 
    using assms card_verts_G_G'
    by auto
  then have 1: "\<forall>x \<in> verts G'. x \<in> set Cy1" 
    using Cy1_def is_uhc_def 
    by fastforce
  have "(x, 0) \<in> verts G'"  "(x, 1) \<in> verts G'"  "(x, 2) \<in> verts G'" 
    using assms verts_G_G' 
    by auto
  then show "(x, 0) \<in> set Cy1"  "(x, 1) \<in> set Cy1"  "(x, 2) \<in> set Cy1" 
    using 1 
    by auto
qed
...
proof -
  ...
  have "vwalk_cycle G' Cy1"
    using Cy1_def assms is_uhc_def 0 
    by force
...
proof -
  ...
  proof(cases "b = 2" )
    ...
    then have "((a, 2), (x, 2)) \<notin> arcs G'"
      ...
      by simp  
    then show ?thesis 
      using True \<open>(y, x, 2) \<in> arcs G'\<close> ab_def
      by auto 
  next
    case False
...
next
  ...
  proof(cases "c= 1")
    ...
    then have "b \<notin> set (to_cycle_hc C)"
      using b_1_not_in_C_not_in_cycle_hc Cons 
      by fast
    then show ?thesis 
      using Cons 
      by (simp add: bc_def) 
...
next
  ...
  next
    ...
    then have 1: "last (to_cycle_hc (C)) = a" 
      ...
      by auto
    have "(a, 1) \<in> set C" 
      using Cons.prems(1) False last_in_set 
      by force
...
next
  ...
  then have 1: "a = (x, 1) \<or> sublist [(x, 1), (x, 2), (y, 0), (y, 1)] C" 
    using sublist_cons_impl_sublist_list
    by force 
...
proof -
  ...
  then have s1: "sublist [(x, 2), (y, 0), (y, 1)] Cy1 \<or> hd Cy1 = (y, 0)"
    using assms distinct_tl_Cy1 sublist_ab_bc_b_not_head 
    by fastforce 
  then show ?thesis 
  proof
...
proof(rule ccontr)
  ...
  then have "(sublist [(x, 1), (x, 2)] Cy1 \<and> sublist [(x, 0), (x, 1)] Cy1) \<or> 
    ...
    using List_Auxiliaries.a_in_Cycle_exists_sublist hd_last_Cy1 length_Cy1(1) post_1_edge  
      C1_def sublist_Cy1_for_x x_1_in_C_x_in_to_cycle_hc x_in_C1_in_verts_G
    by fastforce
...
next
  ...
  then have sublists_x_y: "sublist [(C1!i, 2), (x, 0)] Cy1" 
    using sublist_predecessor "1" distinct_tl_Cy1 hd_last_Cy1 two_sublist_same_first_distinct_tl
    by fastforce
...
proof 
  ...
  then have "(a, b) \<in> arcs G" 
    using assms in_C1_in_arcs x_def
    by blast
...
proof -
  ...
  then show ?thesis proof
    ...
    then have "sublist [(x, 0), (x, 1)] Cy1" 
      using assms(1) in_cy1_in_verts_G list.set_sel(1) neCy1
      by fastforce 
    ...
    then have "sublist [(y, 1), (y, 2)] Cy1" 
      using 1 assms(1) distinct_tl_Cy1 in_cy1_in_verts_G sublist_i_j_last_distinct_tl
      by fastforce 
...
proof 
  ...
  have 2: "set (vwalk_arcs (last C1 # C1)) \<subseteq> arcs G"
    using in_C1_in_arcs_2 assms last_C1_hd_C1_in_arcs 3
    by simp
  then show "set (last C1 # C1) \<subseteq> verts G" "set (vwalk_arcs (last C1 # C1)) \<subseteq> arcs_ends G" 
    "(last C1 # C1) \<noteq> []"  
    using 1 2  arcs_ends_eq_arcs_G in_C1_in_arcs_2 assms 
    by auto  
qed
...
next
  ...
  then have "(\<forall>x \<in> verts G. sublist [(x, 1), (x, 2)] (rev Cyc1) \<and> sublist [(x, 0), (x, 1)] (rev Cyc1))"
    using Cyc1_def sublist_rev_Cy1 
    by blast 

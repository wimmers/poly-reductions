...
proof -
  obtain v p p_cer where pv_def:"poly p" "is_verif v L" "verif_time_bounded v p" "certif_bounded v p_cer"
    "poly p_cer"
    using assms
    by (auto simp add: NP_def is_poly_verif_def poly_verif_time_bounded_def poly_certif_bounded_def)
  ...
  moreover have "\<forall>s. \<exists>t. Ex (big_step_t (v, s) t) \<and> t \<le> p (bit_length (s ''input'') + bit_length (s ''certificate''))" using pv_def(3) 
    by(auto simp add: verif_time_bounded_def)
 ...
 "cons_certif imp_to_sat"  by (auto simp add:cons_certif_def)
  ...
  proof -
    ...
    proof
      ...
      then obtain z  where z_def: "\<forall>r. verif v x z r \<longrightarrow> 0 = r "  "bit_length z \<le> p_cer (bit_length x)"

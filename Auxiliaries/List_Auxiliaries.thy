...
next
  ...
  then show ?case proof(cases "v1 = a")
    ...
    then have "v2 \<in> set ls2"
      using Cons.prems(3) sublist_implies_in_set(2) 
      by force 
    then show ?thesis .
  next
    case False
...
next
  ...
  proof(cases "v1 = a")
    ...
    then have "v2 = hd ls2"
      using Cons 
      by (metis (mono_tags, lifting) Cons_eq_appendI True \<open>ls1 = [a]\<close> sublist3_hd_lists  list.sel(3)) 
    then show ?thesis .
  next
    case False
    have "\<exists>p1 p2. p1@ [v1, v2] @ p2 = (a#L)" 
...
proof -
  ...
  then have "v1 \<in> set ls3" 
    ...
    by auto
  then have "v2 = hd ls2" 
    using assms(1) assms(3) last sublist_set_last_ls1_1 L_def_2 
    by fast
  then show ?thesis .
qed
...
proof -
  have "l1#ls2 = [l1]@ls2" 
    by simp
  then show ?thesis 
    using indices_length_set_ls2 assms 
    by auto
qed 
  ...
  then show ?case proof(cases "a = c")
    ...
    proof(cases "b = c")
      ...
      then have "\<not> distinct Cy" 
        using Cons sublist_implies_in_set_a 
        by metis 
      then show ?thesis 
        using Cons 
        by auto
...
proof(cases "b = hd Cycle")
  ...
  then have "b \<in> set (tl Cycle)" 
    ...
    by fast 
  then show ?thesis 
    using b_in_set_cy_exists_sublist assms  
    by metis 
next
  case False
  then have "b \<in> set (tl Cycle)" 
...
lemma sublist_cons_cons:
  ...
  shows "sublist b C" 
proof -
  have "\<exists>p1 p2. p1 @ (a#b) @ p2 = (c#C)" 
    using sublist_def assms 
    by blast
  then obtain p1 p2 where p_def: "p1 @ (a#b) @ p2 = (c#C)"
    by auto
  then show ?thesis 
  proof(cases "p1 = []")
    case True

...
lemma sublist3_hd_lists:
  ...
  apply(induction L) 
   apply (auto)
  by (metis assms(1, 2) distinct_append hd_append hd_in_set list.sel(1, 3) 
...
proof -
  ...
  then have 1: "sublist [v1, v2] ls2" 
    using sublist_set_ls2_3 assms 1 
    by fast 
  then show ?thesis 
    by (auto simp add: sublist_def)
qed
...
next
  ...
  proof(cases "v1 = a")
    ...
    then have "v2 = hd (tl ls1)" 
      using Cons \<open>tl ls1 \<noteq> []\<close>  \<open>p1 = []\<close>
      by (metis Nil_tl hd_append2 list.sel(3) tl_append2) 
    then show ?thesis 
...
lemma sublist_cons_impl_sublist_list: 
  ...
  apply(auto)
  by (metis append_self_conv2 list.inject list.sel(3) tl_append2)


lemma sublist_cons_impl_sublist: 
  assumes "sublist [a, b] (c#cs)" "a \<noteq> c"
  shows "sublist [a, b] cs"
  using assms sublist_cons_impl_sublist_list 
  by metis
...
next
  ...
  then show ?case proof(cases "v1 = a")
    ...
    then have "v2 = hd L" 
      using Cons 
      using \<open>p1 @ [v1, v2] @ p2 = a # L\<close>
      by (metis Cons_eq_appendI True eq_Nil_appendI list.sel(1) list.sel(3))
    then have "v2 = hd (tl ls1)" 
      using Cons \<open>tl ls1 \<noteq> []\<close> 
      by (metis Nil_tl \<open>p1 = []\<close> hd_append2 list.sel(3) tl_append2) 
    then show ?thesis 
      using sublist_def \<open>p1 = []\<close> \<open>tl ls1 \<noteq> []\<close> \<open>v1 = hd ls1\<close>
      by (metis Cons.prems(4) append_Cons eq_Nil_appendI in_set_replicate 
          list.exhaust_sel replicate_empty)
  next
    case False
    obtain p1 p2 where p_def: "p1@ [v1, v2] @ p2 =a#L" 
      using Cons 
      by (auto simp add: sublist_def)
    then have L_def_2: "L = tl( p1@ [v1, v2] @ p2)" 
      by auto
    ...
    then have "\<exists>p1 p2. p1@[v1, v2]@p2 = tl(ls1)" 
      using Cons 1 2 3 4 5 sublist_def 
      by blast
...
proof -
  have 1: "L = ls3 @ ls2" 
    using assms 
    by simp
  have 2: "v1 \<in> set ls3" 
    using assms 
    by simp
  have 3: "v1 \<noteq> last ls3" 
    using assms 
    by auto
  then obtain p1 p2 where p_def: "p1@ [v1, v2] @ p2 = ls3" 
    using sublist_set_ls1_4 assms 1 2 sublist_def 
    by metis 
  have "v2 \<noteq> l1" 
    using assms 
  ...
  then have "(tl p1)@[v1, v2] @ p2 = ls1" 
    using 2 1 
    by (metis hd_append list.sel(1) list.sel(3) tl_append2) 
  then show ?thesis 
    by (auto simp add: sublist_def)
qed
...
lemma sublist_set_last_ls1_2: 
  ...
  shows "ls1 = [x]"
  using assms apply(induction L arbitrary: ls1 ls2)apply(auto)
  by (metis distinct.simps(2) hd_append2 hd_in_set in_set_conv_decomp_first 
...
next
  ...
  proof(cases "v1 = a")
    ...
    have 2: "v2 \<in> set (a#L)" 
      using assms Cons.prems(3) sublist_implies_in_set(2) 
      by force 
    have "v2 \<noteq> a" 
      using True assms sublist_implies_in_set_a 
      by fastforce
...
lemma sublist_set_last_ls1:
  ...
  shows "v2 = hd ls2"
  using assms sublist_set_last_ls1_3 
  by fast
...
lemma sublist_set_noteq_l1:
  ...
  using assms unfolding sublist_def apply(induction L)apply(auto) 
  by (metis append_self_conv2 assms(2, 4) list.inject list.sel(3) tl_append2)
...
lemma sublist_set_v2_noteq_hd_lists:
  ...
  shows "sublist [v1, v2] (ls1@ls2)" 
  using assms unfolding sublist_def apply(induction L) apply(auto)
  by (metis append_self_conv2 list.sel(1) list.sel(3) tl_append2)  
...
lemma sublist_v1_in_subsets: 
  ...
  using assms 
  apply(induction L arbitrary: l1 l2) 
   apply(auto simp add: sublist_def) 
  by (metis hd_append2 in_set_conv_decomp list.sel(1) list.sel(3) 
...
lemma sublist_v1_hd_v2_hd_tl:  
  ...
  shows "v2 = hd(tl L)"
  using assms 
  apply(induction L arbitrary: v1) 
   apply(auto simp add: sublist_def) 
...
lemma sublist_v1_hd_v2_hd_tl_lists:  
  ...
  shows "vs @ p2 = tl L"
  using assms 
  apply(induction L arbitrary: v vs) 
   apply(auto) 
...
lemma sublist_append:
  ...
  using assms append_eq_append_conv2
  unfolding sublist_def
  by blast
...
lemma sublist_not_cyclic_for_distinct: 
  ...
  using assms unfolding sublist_def 
  apply(auto)
  by (metis assms distinct_append list.sel(1) list.set_intros(1) 
...
lemma distinct_sublist_last_first_of_sublist_false: 
  ...
  using assms unfolding sublist_def 
  apply(auto) 
  by (metis append.right_neutral assms(2) distinct.simps(2) distinct_singleton 
...
lemma distinct_sublist_last_first_of_sublist_false_2: 
  ...
  shows False
  using assms  unfolding sublist_def 
  apply(induction cs) apply(auto split: if_split_asm) 
  by (metis append_self_conv2 last_in_set list.sel(1) list.sel(3) tl_append2)
...
lemma sublist_hd_last_only_2_elems: 
  ...
  shows "cs = [a, b]" 
  using assms unfolding sublist_def 
  apply(induction cs) apply(auto) 
  by (metis append_self_conv2 distinct.simps(2) in_set_conv_decomp last_ConsR 
      last_in_set list.sel(3) tl_append2) 
...
lemma two_sublist_distinct_same_last: 
  ...
  shows "a = b"
  using assms unfolding sublist_def apply(induction L) apply(auto)
  by (metis append_self_conv2 in_set_conv_decomp list.sel(1) list.sel(3) tl_append2)
...
next
  ...
  then have "a= l \<or> sublist [a, b] (l1@l2)" 
    by (metis append_Cons sublist_append_not_eq)
...
lemma distinct_tl_rev_C: 
  ...
  using assms unfolding sublist_def apply(induction C) apply(auto split: if_split_asm) 
  apply (auto simp add: distinct_tl)
   apply (metis Nil_is_rev_conv distinct.simps(2) distinct_rev hd_rev list.collapse)
  using list_set_tl by fastforce
...
lemma b_in_Cycle_exists_sublist:
  ...
  using assms 
  by (metis b_in_set_cy_exists_sublist hd_Cons_tl last_in_set_tl list.sel(2) set_ConsD)
...
lemma sublist_sublist_hd: 
  ...
  apply(induction C) 
   apply(auto simp add: sublist_def) 
  by (metis Nil_is_append_conv hd_Cons_tl hd_append2)  
...
lemma sublist_cons_cons:
  ...
  using assms 
proof -
  obtain p1 p2 where p_def: "p1 @ (a#b) @ p2 = (c#C)"
    using sublist_def assms 
    by blast
  then show ?thesis 
  proof(cases "p1 = []")
    case True
    then have "(a#b)@p2 = (c#C)"
      using p_def 
      by auto
    then have "b@p2 = C" 
      by auto

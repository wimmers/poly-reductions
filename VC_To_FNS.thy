...
proof -
  have 1: "card e = 2" 
    using ugraphE ugraph_def assms by blast 
...
using pre_digraph.cycle_def proof -
  have 1:  "[(v, u), (u, v)] \<noteq> []"
    by auto
  have 2: "pre_digraph.awalk G' v [(v, u), (u, v)] v"
    unfolding pre_digraph.awalk_def proof -
    have 1: "v \<in> verts G'" 
      using wf_digraph_def assms by fastforce
    have 2: "set [(v, u), (u, v)] \<subseteq> arcs G'" 
      using assms by simp
    have 3: "pre_digraph.cas G' v [(v, u), (u, v)] v"
      using pre_digraph_cas_edge assms by fast
  ...
  qed     
  then show ?thesis using 1 2 3 pre_digraph.cycle_def by blast
qed



thm e_in_arcs_not_in_R

lemma head_tail_del_verts: 
  shows "head (del_verts G' E') = head G'" "tail (del_verts G' E') = tail G'"
   apply(induction E' arbitrary: G') 
  by(auto simp add: pre_digraph.head_del_vert pre_digraph.tail_del_vert) 

lemma wf_digraph_del_verts:
  assumes "wf_digraph G'"
  shows "wf_digraph (del_verts G' E')"
  using assms apply(induction E' arbitrary: G')
  by(auto simp add: wf_digraph.wf_digraph_del_vert) 
...
proof(rule ccontr)
  ...
  then have "u \<notin> set R \<and> v \<notin> set R" by auto
  then have 1: "(v, u) \<in> arcs (del_verts G R)"  "(u, v) \<in> arcs (del_verts G R)" "u \<noteq> v"
    using G_def_3 e_in_arcs_not_in_R assms by force+
  have 2: "tail (del_verts G R) = fst" "head (del_verts G R) = snd"
    using G_def_3 by(auto simp add: head_tail_del_verts)   
  have "wf_digraph (del_verts G R)" 
    using wf_digraph_G_2 wf_digraph_del_verts by auto
  then have "pre_digraph.cycle (del_verts G R) [(v, u), (u, v)]"
    using edge_cycle 1 2 by fast 
  then show False using R_def is_fns_def by blast
qed
...
proof -
  have "set R \<subseteq> \<Union> E" 
    using R_def G_def_3 by auto
  then show "Cov \<subseteq> \<Union> E" using Cov_def by simp
next
  have "length R \<le> k"
    using R_def G_def_3 
    by linarith
  then show "card Cov \<le> k" 
    using Cov_def card_length dual_order.trans by blast  
next
  have "\<forall>e\<in>E. \<exists>v\<in>Cov. v \<in> e" proof 
    fix e assume 1: "e \<in> E"
    then obtain u v where 2: "e = {u, v}" "u \<noteq> v"
      using e_in_E_e_explicit by metis 
    then have "(u, v) \<in> arcs G"
      using G_def_3 1 by auto
    then have "u \<in> set R \<or> v \<in> set R" 
      using in_arcs_one_in_R 2 by simp
    then show "\<exists>v\<in> Cov. v\<in> e"
      using Cov_def 2 by auto
  qed
  then show "is_vertex_cover E Cov"
    using is_vertex_cover_def by auto
qed

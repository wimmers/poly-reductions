...
proof -
  have 1: "hd p = last p" "distinct (tl p)" "vwalk p G" "length p \<ge> 2" 
    using vwalk_cycle_def assms by blast+
  then have 2: "distinct (tl (rev p))"
    using distinct_tl_rev_C by blast
  then have 3: "vwalk (rev p) G"
    using assms 1
    by (simp add: vwalk_rev_ex) 
  have 4: "hd (rev p) = last (rev p)" 
    using 1 hd_rev last_rev by fastforce 
  have 5: "length (rev p) \<ge> 2"
    using 1 by simp
  then show ?thesis using 1 2 3 4 vwalk_cycle_def by blast 
qed
...
proof -
  have 1: "u \<in> verts G" using assms by auto
  have 2: "set [] \<subseteq> arcs G" by simp
  have 3: "pre_digraph.cas G u [] u"
    by (simp add: pre_digraph.cas.simps(1)) 
  then show ?thesis using 1 2 pre_digraph.awalk_def by metis
qed
...
next
  ...
  then have a3: "... =  (tail G (u,v) = u \<and> pre_digraph.cas G (head G (u, v)) (vwalk_arcs (v # es)) (last (v # es)))"
    using Cons by(auto simp add: pre_digraph.cas.simps) 
  ...
  then have 1: "pre_digraph.cas G (hd (u # v # es)) (vwalk_arcs (u # v # es)) (last (u # v # es)) = 
    ...
    using a1 a2 a3 a4 a5 by argo
  show ?case proof(cases "length es \<le>1")
    case True
    then show ?thesis proof(cases "length es = 1")
      case True
    ...
    next
      ...
      then have "length es = 0" 
        using True order.not_eq_order_implies_strict by auto 
      then have "es = []" 
        by simp
      then have "last (v # es) = v" "vwalk_arcs (v#es) = []" 
        by auto
      then show ?thesis using a5 1 
        by (simp add: pre_digraph.cas.simps(1)) 
    qed
  next
    case False
...
lemma vwalk_cycle_impl_cycle: 
  ...
  using assms unfolding pre_digraph.cycle_def vwalk_cycle_def
...
proof -
  have 1: "vwalk_arcs c \<noteq> []" 
    using assms pre_digraph.cycle_def by force 
  have "\<exists>u. pre_digraph.awalk G u (vwalk_arcs c) u" 
    using assms pre_digraph.cycle_def by metis
  ...
  then have "fst (hd (vwalk_arcs c)) = u"  "snd (last (vwalk_arcs c)) = u"
    using hd_pre_digraph_cas last_pre_digraph_cas 1 assms by fastforce+
  then have "fst (hd (vwalk_arcs c)) = snd (last (vwalk_arcs c))"
    by simp
  then show ?thesis using hd_first_edge tail_last_edge assms by metis
qed
...
proof -
  have "\<forall>u. pre_digraph.awalk_verts G u (vwalk_arcs c) = c"
    using assms at_least_to_nodes_vwalk_arcs_awalk_verts by force
  then have 1: "distinct (tl c)" 
    using assms pre_digraph.cycle_def by metis
  have 3: "vwalk c G" using assms 
    by (metis \<open>\<forall>u. pre_digraph.awalk_verts G u (vwalk_arcs c) = c\<close> wf_digraph.awalk_imp_vwalk wf_digraph.cycle_conv) 
  have 4: "hd c = last c"
    using hd_last_cycle assms by metis
  then show ?thesis using 1 3 4 vwalk_cycle_def assms by blast 
qed
